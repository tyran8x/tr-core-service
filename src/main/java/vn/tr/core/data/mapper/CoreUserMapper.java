...
public interface CoreUserMapper {
	...
	CoreUserData toData(CoreUser coreUser);
	
	List<CoreUserData> toData(List<CoreUser> coreUsers);
	...
	// Giá trị mặc định
	CoreUser toEntity(CoreUserData coreUserData);
	
	// Cập nhật Entity từ DTO
	@Mapping(target = "id", ignore = true)
	@Mapping(target = "username", ignore = true) // Không cho phép đổi username
	@Mapping(target = "hashedPassword", ignore = true)
	void updateEntityFromData(CoreUserData coreUserData, @MappingTarget CoreUser coreUser);
