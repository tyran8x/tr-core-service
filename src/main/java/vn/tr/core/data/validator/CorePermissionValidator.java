...
public class CorePermissionValidator implements Validator {
	...
	public boolean supports(@NonNull Class<?> clazz) {
		return CorePermissionData.class.isAssignableFrom(clazz);
	}
	...
	public void validate(@Nullable Object target, @Nullable Errors errors) {
		...
		if (target == null) {
			errors.reject(ERROR_TARGET_NULL, "Đối tượng CorePermissionData không được null.");
			return;
		}
		
		CorePermissionData data = (CorePermissionData) target;
...
//		}
	...
	private boolean isDuplicate(Long id, String value, String appCode, boolean isCode) {
		if (id != null) {
			return isCode
					? corePermissionService.existsByIdNotAndCodeIgnoreCaseAndAppCode(id, value, appCode)
					: corePermissionService.existsByIdNotAndNameIgnoreCaseAndAppCode(id, value, appCode);
		}
		return isCode
				? corePermissionService.existsByCodeIgnoreCaseAndAppCode(value, appCode)
				: corePermissionService.existsByNameIgnoreCaseAndAppCode(value, appCode);
	}
}
