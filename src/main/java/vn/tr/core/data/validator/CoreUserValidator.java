...
public class CoreUserValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (checkEmailExits(object.getId(), object.getEmail())) {
	...
	private boolean checkEmailExits(Long id, String email) {
		if (CharSequenceUtil.isNotBlank(email)) {
			if (Objects.nonNull(id)) {
				return coreUserService.existsByIdNotAndEmailIgnoreCaseAndDaXoaFalse(id, email);
			}
			return coreUserService.existsByEmailIgnoreCaseAndDaXoaFalse(email);
		}
		return false;
	}
}
