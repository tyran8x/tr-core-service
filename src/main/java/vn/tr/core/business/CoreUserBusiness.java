...
/**
 * Lớp Business (Facade) điều phối các nghiệp vụ phức tạp nhất liên quan đến
 * Quản lý Người dùng (CoreUser). Đây là module trung tâm, tương tác với nhiều
 * service khác để đồng bộ hóa toàn bộ thông tin người dùng.
 *
 * @author tyran8x
...
public class CoreUserBusiness {
     ...
     * @param appCodeContext Ngữ cảnh ứng dụng của người thực hiện. Null nếu là
     * Super Admin.
     *
    ...
    /**
     * Phương thức "upsert" lõi, điều phối việc tạo/cập nhật và đồng bộ hóa tất
     * cả các quan hệ.
     */
    ...
    /**
     * Đồng bộ hóa tất cả các mối quan hệ của User, sử dụng
     * AssociationSyncHelper và các Domain Service.
     */
    ...
    /**
     * Map một danh sách Entities sang DTOs và tải các quan hệ một cách tối ưu
     * (tránh N+1).
     */
    ...
    private boolean hasPermission(String targetUsername, String appCodeContext) {
        if (appCodeContext == null) {
            return true; // Super Admin có mọi quyền
