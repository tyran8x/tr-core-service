...
public interface IAuthStrategy {
	...
	String DEFAULT_REGISTER_STRATEGY_BEAN_NAME = "password" + BASE_NAME;
	 ...
	 */
	static LoginResult executeLogin(LoginBody loginBody, String appCode) {
		...
		return instance.performLogin(loginBody, appCode);
	}
	
	/**
	 * (SỬA) Phương thức instance để thực thi logic đăng nhập cụ thể. Tên mới: performLogin.
	 */
	LoginResult performLogin(LoginBody loginBody, String appCode);
	 ...
	 */
	static void executeRegister(RegisterBody registerBody) {
		...
		// Gọi phương thức instance để thực thi logic
		instance.performRegistration(registerBody);
	}
	
	/**
	 * (SỬA) Phương thức instance để thực thi logic đăng ký cụ thể. Tên mới: performRegistration.
	 */
	default void performRegistration(RegisterBody registerBody) {
